...
parser.add_argument('--train_neg', type=str, default='CurNS', help='use different negative sampling for training')
...
from Ring_dens import ring, policy
...
args.thres_outer_policy, args.thres_inner_policy = ring.create_negative_annealing_schedule(args)
min_src, max_src = int(min(train_df.src.values)), int(max(train_df.src.values))
if np.intersect1d(train_df.src.values, train_df.dst.values).size > 0:
...
for e in range(args.num_epoch):
    ...
    for _, rows in tqdm(train_df.groupby(random_chunk.split_batch())):
        ...
        all_nodes = cand_nodes.flatten()
        cand_time = np.repeat(batch_time, args.M)

        # Add random negative samples.
        if args.add_random:
            neg_samples += args.M
            _, batch_rand_nodes = train_neg_sampler.random_sample(batch_rows*args.M)
            all_nodes = np.concatenate((all_nodes, batch_rand_nodes))
            cand_time = np.concatenate((cand_time, np.repeat(batch_time, args.M)))
        ...
        pred_pos, pred_neg = model.edge_predictor(torch.cat([s_e, p_e, n_e]), neg_samples=args.M)
        ...
        gated_p_e_r = p_e * gate_p    # [batch_size, channel]
        ...
        # Selection function (simplified form used).
        n_e_sel = torch.sigmoid(s_temb.unsqueeze(1) * (n_interact_temb * n_occurr_temb).view(batch_rows, args.M, dim_out)) * ((1 - min(1, e / args.stop_irr_epoch)) * n_e - gated_n_e)    # [batch_size, n_negs, channel]

        # Sample based on Adaptive Pi.
        _, pred_nsel = model.edge_predictor(torch.cat([s_e, s_e, n_e_sel.reshape(batch_rows*args.M, dim_out)]), neg_samples=args.M)
        pred_r = pred_neg.flatten()[torch.argsort(pred_nsel.flatten(), descending=True)[inner_idx:outer_idx]].view(num_hard,1)

        # Train using selected and random negative samples.
        loss = creterion(pred_pos, torch.ones_like(pred_pos))
        if args.add_random:
            loss += (1-rand_prop) * creterion(pred_r, torch.zeros_like(pred_r))
            _, pred_rand = model.edge_predictor(torch.cat([s_e, s_e, emb[(2+args.M)*batch_rows:]]), neg_samples=args.M)
            loss += rand_prop * creterion(pred_rand, torch.zeros_like(pred_rand))
        else:
            loss += creterion(pred_r, torch.zeros_like(pred_r))

        # Contrastive loss.
        row_idx = torch.arange(0, batch_rows)
        sel_col = torch.argsort(pred_nsel.view(batch_rows, args.M), descending=True)[:, 0]
        sel_e = n_e[row_idx, sel_col]
        gated_neg_e_r = sel_e * gate_n[row_idx, sel_col]    # [batch_size, channel]
        gated_neg_e_ir = sel_e - gated_neg_e_r
        gated_pos_scores_r, gated_neg_scores_r = model.edge_predictor(torch.cat([s_e, gated_p_e_r, gated_neg_e_r]))
        gated_pos_scores_ir, gated_neg_scores_ir = model.edge_predictor(torch.cat([s_e, gated_p_e_ir, gated_neg_e_ir]))
