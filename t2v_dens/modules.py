...
from memorys import *
from layers import *
from .periodic_activations import Time2Vec

class GeneralModel(torch.nn.Module):

    def __init__(self, dim_node, dim_edge, sample_param, memory_param, gnn_param, train_param, combined=False, args=None):
        ...
        self.user_gate = torch.nn.Linear(gnn_param['dim_out'], gnn_param['dim_out'])
        self.item_gate = torch.nn.Linear(gnn_param['dim_out'], gnn_param['dim_out'])
        self.neg_gate = torch.nn.Linear(gnn_param['dim_out'], gnn_param['dim_out'])
        self.pos_gate = torch.nn.Linear(gnn_param['dim_out'], gnn_param['dim_out'])
        self.t2v_1 = Time2Vec("sin", args.t2v_hiddem_dim, gnn_param['dim_out']).to(args.device)
        self.t2v_2 = Time2Vec("sin", args.t2v_hiddem_dim, gnn_param['dim_out']).to(args.device)
        self.t2v_3 = Time2Vec("sin", args.t2v_hiddem_dim, gnn_param['dim_out']).to(args.device)
    ...
    def backup_memory_updater(self):
        return {
            'last_updated_memory': self.memory_updater.last_updated_memory.clone(),
            'last_updated_ts': self.memory_updater.last_updated_ts.clone(),
            'last_updated_nid': self.memory_updater.last_updated_nid.clone(),
        }
    
    def load_memory_updater(self, updater_state):
        self.memory_updater.last_updated_memory = updater_state['last_updated_memory']
        self.memory_updater.last_updated_ts = updater_state['last_updated_ts']
        self.memory_updater.last_updated_nid = updater_state['last_updated_nid']
    ...
    def get_t2v1(self, time_tensor):
        time_emb = self.t2v_1(time_tensor)
        return time_emb
    
    def get_t2v2(self, time_tensor):
        time_emb = self.t2v_2(time_tensor)
        return time_emb
    
    def get_t2v3(self, time_tensor):
        time_emb = self.t2v_3(time_tensor)
        return time_emb
