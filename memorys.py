...
class MailBox(torch.nn.Module):
    ...
    def __init__(self, memory_param, num_nodes, dim_edge_feat, _node_memory=None, _node_memory_ts=None,_mailbox=None, _mailbox_ts=None, _next_mail_pos=None, _update_mail_pos=None):
        ...
        node_memory = torch.zeros((num_nodes, memory_param['dim_out']), dtype=torch.float32) if _node_memory is None else _node_memory
        node_memory_ts = torch.zeros(num_nodes, dtype=torch.float32) if _node_memory_ts is None else _node_memory_ts
        mailbox = torch.zeros((num_nodes, memory_param['mailbox_size'], 2 * memory_param['dim_out'] + dim_edge_feat), dtype=torch.float32) if _mailbox is None else _mailbox
        mailbox_ts = torch.zeros((num_nodes, memory_param['mailbox_size']), dtype=torch.float32) if _mailbox_ts is None else _mailbox_ts
        next_mail_pos = torch.zeros((num_nodes), dtype=torch.long) if _next_mail_pos is None else _next_mail_pos
        self.update_mail_pos = _update_mail_pos
        self.device = torch.device('cpu')

        # Register buffer to save torch.Tensor.
        self.register_buffer('node_memory', node_memory)
        self.register_buffer('node_memory_ts', node_memory_ts)
        self.register_buffer('mailbox', mailbox)
        self.register_buffer('mailbox_ts', mailbox_ts)
        self.register_buffer('next_mail_pos', next_mail_pos)
    ...
    def prep_input_mails(self, mfg, use_pinned_buffers=False):
        for i, b in enumerate(mfg):
            if use_pinned_buffers:
                ...
                b.srcdata['mem'] = self.node_memory[b.srcdata['ID'].cpu().long()].cuda()
                b.srcdata['mem_ts'] = self.node_memory_ts[b.srcdata['ID'].cpu().long()].cuda()
                b.srcdata['mem_input'] = self.mailbox[b.srcdata['ID'].cpu().long()].cuda().reshape(b.srcdata['ID'].shape[0], -1)
                b.srcdata['mail_ts'] = self.mailbox_ts[b.srcdata['ID'].cpu().long()].cuda()
